...
typedef libusb_get_device_descriptor_c = ffi.Int32 Function(ffi.Pointer<ffi.Void> dev, ffi.Pointer<ffi.Void> desc);
typedef libusb_get_device_descriptor_dart = int Function(ffi.Pointer<ffi.Void> dev, ffi.Pointer<ffi.Void> desc);

class UsbPacketNative extends StatefulWidget {
  @override
  _UsbPacketNativeState createState() => _UsbPacketNativeState();
}

class _UsbPacketNativeState extends State<UsbPacketNative> {
  // 颜色常量定义
  ...
  // libusb函数指针
  late final libusb_init_dart _libusb_init;
  late final libusb_exit_dart _libusb_exit;
  late final libusb_get_device_list_dart _libusb_get_device_list;
  late final libusb_free_device_list_dart _libusb_free_device_list;
  late final libusb_get_device_descriptor_dart _libusb_get_device_descriptor;
  ...
  void _initializeLibUSB() {
    try {
      // 获取libusb函数指针
      _libusb_init = _libusb.lookupFunction<libusb_init_c, libusb_init_dart>('libusb_init');
      _libusb_exit = _libusb.lookupFunction<libusb_exit_c, libusb_exit_dart>('libusb_exit');
      _libusb_get_device_list = _libusb.lookupFunction<libusb_get_device_list_c, libusb_get_device_list_dart>('libusb_get_device_list');
      _libusb_free_device_list = _libusb.lookupFunction<libusb_free_device_list_c, libusb_free_device_list_dart>('libusb_free_device_list');
      _libusb_get_device_descriptor = _libusb.lookupFunction<libusb_get_device_descriptor_c, libusb_get_device_descriptor_dart>('libusb_get_device_descriptor');

      final contextPtr = calloc<ffi.Pointer<ffi.Void>>();
      final result = _libusb_init(contextPtr);
  ...
  Future<void> _scanUsbDevices() async {
    ...
    try {
      final deviceListPtr = calloc<ffi.Pointer<ffi.Pointer<ffi.Void>>>();
      final deviceCount = _libusb_get_device_list(_context!, deviceListPtr);
      ...
      // 简化的设备描述符结构
      final descSize = 18; // USB设备描述符大小
      final descPtr = calloc<ffi.Uint8>(descSize);
      
      for (int i = 0; i < deviceCount; i++) {
        final device = deviceList[i];
        
        try {
          final result = _libusb_get_device_descriptor(device, descPtr.cast());
          if (result == 0) {
            final vendorId = descPtr[8] | (descPtr[9] << 8);
            final productId = descPtr[10] | (descPtr[11] << 8);
            
            devices.add(UsbDevice(
              deviceId: '${vendorId.toRadixString(16).toUpperCase()}:${productId.toRadixString(16).toUpperCase()}',
              vendorId: vendorId,
              productId: productId,
              vendorName: '厂商 0x${vendorId.toRadixString(16).toUpperCase()}',
              productName: '产品 0x${productId.toRadixString(16).toUpperCase()}',
            ));
          }
        } catch (e) {
          _logSystem('获取设备描述符失败: $e');
        }
      }

      calloc.free(descPtr);
      _libusb_free_device_list(deviceList, 1);
      calloc.free(deviceListPtr);
